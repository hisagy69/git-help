...
используется, чтобы перепривязать к другому репозиторию
...
если указать флаг -D, то произойдет удаление ветки (надо находиться в другой ветке) даже если ветка не объединена с веткой master
если указать флаг -d, то произойдет удаление ветки (надо находиться в другой ветке), если ветка является частью master
